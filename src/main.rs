...
use environments::{frozen_lake::*, gym_adapter::*, simple_golf::*};
use gymnasium::*;
use mdps::mdp::*;
use mdps::mdp_solver::*;
use mdps::solvers::{policy_iteration::*, value_iteration::*};
use serde_json::{to_value, Value};
use std::collections::HashMap;

fn main() {
    let mdp = &SimpleGolf::new(0.9) as &dyn Mdp;
    let mut p = PolicyIteration::new(mdp, 0., 0);
    let delta = p.exec(1e-4, Some(2));
    println!(
        "delta = {delta:?} => {:?}",
        (0..mdp.n_s()).map(|s| p.v_star(s)).collect::<Vec<_>>()
    );

    let mdp = &FrozenLake::new(0.9) as &dyn Mdp;
    let mut v = ValueIteration::new(mdp, 0.);
    let delta = v.exec(1e-4, Some(2));
    println!(
        "delta = {delta:?} => {:?}",
        (0..mdp.n_s()).map(|s| v.v_star(s)).collect::<Vec<_>>()
    );
    ...
    let env = gc
        .make_env("FrozenLake-v1", Some(400), Some(false), Some(true), kwargs)
        .unwrap();
    ...
    let v_star = (0..mdp.n_s()).map(|s| p.v_star(s)).collect::<Vec<_>>();
    println!("{v_star:?}");
    let pi_star = (0..mdp.n_s()).map(|s| p.pi_star(s)).collect::<Vec<_>>();
    println!("{pi_star:?}");
    let mut q_star = Vec::new();
    for s in 0..mdp.n_s() {
        for a in 0..mdp.n_a() {
            q_star.push(p.q_star(s, a))
        }
    }
    println!("{q_star:?}");
